...
if switch == 0:
    ...
    weights_for_LipSDP, biases_for_LipSDP = net_vanilla.extract_weights()
elif switch == 1:
    weights, biases = create_parameter_lists(net_L2)
    weights_for_LipSDP, biases_for_LipSDP = net_L2.extract_weights()
elif switch == 2:
    weights, biases = create_parameter_lists(net_L1)
    weights_for_LipSDP, biases_for_LipSDP = net_L1.extract_weights()
...
#Ergebnis LipBaB
print("Final Lipschitz estimation LipBaB:", tp.Lub)
execution_time = (time.time() - start_time)
#print('Execution time in seconds: ' + str(execution_time))

#obere Schranke Combettes
hidden_layer_dim = [20]
theta_m = Theta_m(weights, hidden_layer_dim)
print("upper bound Combettes", theta_m)

# obere Schranke LipSDP-Neuron
net_dim = [2, 20, 3]
LipSDP_neuron = lipSDP(weights_for_LipSDP, net_dim, 'neuron')
print("Schätzung LipSDP-Neuron", LipSDP_neuron)

# LipSDP-Layer
LipSDP_layer = lipSDP(weights_for_LipSDP, net_dim, 'layer')
print("Schätzung LipSDP-Layer", LipSDP_layer)
...
#write to Excel
# change value
ws.cell(Round+1,3).value = tp.Lub
ws.cell(Round+1,7).value = theta_m
ws.cell(Round+1,8).value = LipSDP_neuron
ws.cell(Round+1,9).value = LipSDP_layer
