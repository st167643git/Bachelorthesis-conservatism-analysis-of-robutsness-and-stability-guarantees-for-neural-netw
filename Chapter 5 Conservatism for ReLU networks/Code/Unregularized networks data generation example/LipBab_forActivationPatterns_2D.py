...
import math
import sys
from Create_Parameter_Lists_2D import *

# load from Excel
name_str = 'Daten_trainedNetwork_2DToyExample_1x20.xlsx'
...
# function for compromising actvp_arrray to a more readable format
def activationpatterns(pattern_array):
    all_patterns = []
    for pattern in pattern_array:
        ...
        for layer in pattern:
            layer_list = []
            if (layer_counter != 1) & (layer_counter != len(pattern)):
                for neuron in layer:
                    layer_list.append(neuron[1])
                pattern_list.append(layer_list)
            layer_counter += 1
...
def branch(p,sgn):
    ...
    if res['status']=='optimal':
        ...
        if len(pb.ast_ns)==0:
            bol = pb.ast_ns in actvp_array
            if not bol:
                actvp_array.append(pb.actvp)
            glb=max(glb,pb.Lub)
...
if switch == 0:
    ...
    namestring = '2D_NomModel_1x20_stateDict_L1' + str(Round) + '.pth'
    net_vanilla.load_state_dict(torch.load(namestring))
    net_vanilla.eval()
    weights, biases = create_parameter_lists(net_vanilla)
elif switch == 1:
    weights, biases = create_parameter_lists(net_L2)
elif switch == 2:
    weights, biases = create_parameter_lists(net_L1)
...
bnds=[[-1e35,1e35] for i in range(ls[0])]
...
patterns = activationpatterns(actvp_array)
print("All possible Activation patterns:", patterns)
number_of_patterns = len(patterns)
print("Number of Activation patterns:", number_of_patterns)

hidden_layer_dim = [20]

N_ones_array = []
for pattern in patterns:
    n_ones = np.count_nonzero(pattern)
    N_ones_array.append(n_ones)
average_N_ones = np.mean(N_ones_array)
average_N_zeros = np.sum(hidden_layer_dim) - average_N_ones

always_inactive_neurons = np.ones((1, 20))
for pattern in patterns:
    for index_layer in range(len(pattern)):
        layer = pattern[index_layer]
        for index_neuron in range(len(layer)):
            value = layer[index_neuron]
            if value == 1:
                always_inactive_neurons[index_layer, index_neuron] = 0
print("Always inactive neurons",always_inactive_neurons)
n_always_inactive = np.count_nonzero(always_inactive_neurons)

always_active_neurons = np.ones((1, 20))
for pattern in patterns:
    for index_layer in range(len(pattern)):
        layer = pattern[index_layer]
        for index_neuron in range(len(layer)):
            value = layer[index_neuron]
            if value == 0:
                always_active_neurons[index_layer, index_neuron] = 0
print("Always active neurons", always_active_neurons)
n_always_active = np.count_nonzero(always_active_neurons)


#load Round safed in (1,1)
Round = ws.cell(1,1).value
# change value
ws.cell(Round+1,4).value = number_of_patterns
ws.cell(Round+1,5).value = n_always_active
ws.cell(Round+1,6).value = n_always_inactive
ws.cell(Round+1,10).value = average_N_zeros
ws.cell(Round+1,11).value = average_N_ones
